...
# ------------------------------------------------------
# ✅ Stable across DPI (uses fill ratio instead of pixel counts)
# ✅ Calibration sliders for ROI
# ✅ Reads student code + grades + Excel export
# ✅ Batch pages for Cloud
...
# ✅ tuned for your sheet (do NOT widen too much)
BASE = TemplateConfig(
    id_roi=(600, 140, 320, 420),   # ✅ stable ROI for ID grid
    id_digits=4,
    id_rows=10,
    q_blocks=[
...
st.title("تصحيح ببل شيت (نسخة ثابتة – Fill Ratio)")
...
with col1:
    ...
    debug = st.checkbox("Debug (يعرض القص للصفحة الأولى)", True)
...
if roster_file:
    ...
    # ✅ لو أكوادك 4 خانات دائماً، فعّل هذا:
...
if st.button("🚀 ابدأ التصحيح"):
    ...
    st.download_button("⬇️ تحميل Excel",
                       data=buf.getvalue(),
